...
#define BUF_SIZE 16
#define STEP_PRECISION 8 // BUF_SIZE - STEP_PRECISION - 1 = maximum number of digits left of decimal
...
void loop(void)
{
  int newWeight = (digitalRead(buttonInputPin) == HIGH) ? 10 : 0;
  if (newMeasure) {
    lastTime = now();
    newMeasure = false;
  } else {
...
void logStepsToServer(float steps)
{
  /* Convert float to string in the calling frame */
  char buf[BUF_SIZE];
  memset(buf, 0, sizeof(buf));
  char *str;
  dtostrf(steps, 0, STEP_PRECISION, buf); // See warning for STEP_PRECISION above
  ...
  if (www.connected()) {
    ...
    www.fastrprint(PRIVATE);
    www.fastrprint(F("&steps="));
    www.fastrprint(buf);
